...
function ChartManager() {
	...
	var names = [{'url': this.so, 'name': this.companyName + ' sentiment'}],
	...
	this.createChart = function() {
		...
		$.each(names, function(i, name) {
			var timeSerieURL = './timeserie?so='+ name.url +'&startdate='+startDate+'&enddate='+endDate+'&topic='+topic+'&indicator='+indicator+'&callback=?';

			$.getJSON(timeSerieURL,	function(data) {
				
				var hc = $('#container').highcharts();
				
				hc.series[i].setData(data);
		...
		var volumeTimeSerieURL = './timeserievolume?so='+this.so+'&startdate='+startDate+'&enddate='+endDate+'&topic='+topic+'&indicator='+indicator+'&callback=?';
...
//			}
		...
		function createChart() {
	        ...
	        $('#container').highcharts({
                ...
                subtitle: {
	                text: 'daily aggregated sentiment, moving average',
	                x: -20,
	                y: 30
	            },
