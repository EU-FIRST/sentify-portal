package eu.first.sentify.companypage.stock;
...
public class StockPageServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		if (request.getParameter("so") == null) {
			response.sendRedirect(request.getServletPath() + "?so=comp_GOOGLE_INC");
			...
			if (company != null) {
				...
				String targetPage = "stocks.jsp";
				
				if (servletMode.endsWith("/stocks") == true) {
					targetPage = "stocks.jsp";
				}
				if (servletMode.endsWith("/stocks-fuzzy") == true) {
					targetPage = "stocks-fuzzy.jsp";
				}
				if (servletMode.endsWith("/stocks-fuzzy2") == true) {
					targetPage = "stocks-fuzzy2.jsp";
				}
				if (servletMode.endsWith("/stocks-twitter") == true) {
					targetPage = "stocks-twitter.jsp";
				}
