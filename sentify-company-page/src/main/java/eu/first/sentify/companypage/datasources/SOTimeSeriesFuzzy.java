...
import eu.first.commons.ws.client.SentimentServiceStub.GetSentimentIndexScoreObjectLastNDaysFuzzy;
import eu.first.commons.ws.client.SentimentServiceStub.GetSentimentIndexScoreObjectLastNDaysFuzzyResponse;
...
import eu.first.commons.ws.client.SentimentServiceStub.GetSentimentIndexScoreObjectPerDayFuzzy;
import eu.first.commons.ws.client.SentimentServiceStub.GetSentimentIndexScoreObjectPerDayFuzzyResponse;
...
public class SOTimeSeriesFuzzy extends HttpServlet {
			...
			if (ts != null) {
				...
				try {
					...
					if (startDate.isEmpty() == true) {
						...
						GetSentimentIndexScoreObjectLastNDaysFuzzy req = new GetSentimentIndexScoreObjectLastNDaysFuzzy();
				    	...
				    	GetSentimentIndexScoreObjectLastNDaysFuzzyResponse serviceResponse;
						
						serviceResponse = service.getSentimentIndexScoreObjectLastNDaysFuzzy(req);
						score = serviceResponse.get_return();
					} else {
						GetSentimentIndexScoreObjectPerDayFuzzy req = new GetSentimentIndexScoreObjectPerDayFuzzy();
						req.setSentiment_object_uri("http://project-first.eu/ontology#" + request.getParameter("so"));
				    	...
				    	GetSentimentIndexScoreObjectPerDayFuzzyResponse serviceResponse;
						
						serviceResponse = service.getSentimentIndexScoreObjectPerDayFuzzy(req);
						score = serviceResponse.get_return();
					}
					...
					if (score != null) {
				    	...
				    	for (ScoreByDate s : score) {
				    		...
				    		sb.append("[" + s.getDate().getTime() + ", " + s.getPositiveFuzzy() + ", " + s.getNegativeFuzzy() + ", " + s.getVolumen() + "],\n");
				    	}
